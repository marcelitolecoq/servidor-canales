...
def get_abc_url():
    try:
        r = requests.get(f"https://www.dailymotion.com/player/metadata/video/{ABC_VIDEO_ID}?embedder=https://www.abc.com.py", headers=HEADERS, timeout=10)
...
def canales():
    if not cache["m3u"] or (time.time() - cache["ts"]) > 300:
        abc = get_abc_url() or ""
        cache["m3u"] = f"""#EXTM3U
...
http://edge02-fdo-py.cvattv.com.ar/live/c5eds/TELEFUTURO_C4/verimatrix_rotating_FTA/TELEFUTURO_C4-video=3000000-audio_20000=144800.m3u8"""
        cache["ts"] = time.time()
    return Response(cache["m3u"], mimetype="application/x-mpegurl")
